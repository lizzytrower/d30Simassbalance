...
%constant slow continental growth model, following Greber et al. (2017).
...
claytime = flipud(Grebercontmodel(:,1));
f_fspar = flipud(Grebercontmodel(:,2)/100);
f_ol = flipud((Grebercontmodel(:,3) + Grebercontmodel(:,4))/100);
...
Claysims = 1*randn(n,length(xfall)) + F2(xfall);
...
saveas(gcf,'Troweretalmodelv3_greber_fch','epsc')
...
saveas(gcf,'Troweretalmodelv3_greber_fclay','epsc')
...
saveas(gcf,'Troweretalmodelv3_greber_dClay','epsc')
